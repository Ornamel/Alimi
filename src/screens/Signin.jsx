    ...
    <div className="scr" style={sx(ctx.gridAuth)}>
      ...
      {ctx.notIsMobile && (
        <div style={sx('background: #0C1712; padding: 56px 60px; display: flex; flex-direction: column; justify-content: space-between; color: #C9D2CC;')}>
          ...
          <div>
            <h2 style={sx("font-family: 'Space Grotesk'; font-weight: 600; font-size: 34px; line-height: 1.15; color: #F5F2EB; margin: 0 0 18px; letter-spacing: -0.01em;")}>Good to see you again.</h2>
            <p style={sx('font-size: 16px; line-height: 1.6; color: #9FB0A6; max-width: 400px;')}>Your agents kept working while you were away. Sign in to see what came in.</p>
      ...
      <div style={sx('background: #F5F2EB; display: flex; align-items: center; justify-content: center; padding: 40px;')}>
        <div style={sx('width: 100%; max-width: 380px;')}>
          <h1 style={sx("font-family: 'Space Grotesk'; font-weight: 600; font-size: 30px; margin: 0; letter-spacing: -0.01em;")}>Welcome back.</h1>
          <p style={sx('margin: 10px 0 28px; color: #6E7A73; font-size: 15px;')}>Sign in to your Alimi dashboard.</p>
          <div style={sx('display: flex; flex-direction: column; gap: 14px;')}>
            ...
            <div>
              <div style={sx('display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;')}>
                <label style={sx('font-size:13px; font-weight:600;')}>Password</label>
                <button onClick={ctx.goForgotPw} style={sx('background: none; border: none; color: #0B7A4B; font-size: 12.5px; font-weight: 600; cursor: pointer; padding: 0;')}>Forgot password?</button>
              </div>
              <input type="password" placeholder="••••••••" style={sx('width: 100%; border: 1px solid #E4DFD3; background: #fff; border-radius: 11px; padding: 12px 15px; font-size: 15px; outline: none;')} />
            </div>
            <button onClick={ctx.signInToAgents} style={sx('margin-top: 6px; width: 100%; background: #0B7A4B; color: #fff; border: none; padding: 14px; border-radius: 999px; font-size: 15px; font-weight: 600; cursor: pointer; box-shadow: 0 3px 10px rgba(11,122,75,0.25);')}>Sign in →</button>
          </div>
          <p style={sx('margin: 18px 0 0; font-size: 14px; color: #6E7A73; text-align: center;')}>Don't have an account? <button onClick={ctx.goSignup} style={sx('background: none; border: none; color: #0B7A4B; font-size: 14px; font-weight: 600; cursor: pointer; padding: 0;')}>Sign up</button></p>
        </div>
      </div>
    </div>
  );
